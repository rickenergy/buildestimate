import type { Dict } from "./en";

export const es: Dict = {
  appName: "ContractorOS AI",
  tagline: "Fotos, medidas o voz → presupuesto profesional en minutos",

  nav: { home: "Inicio", estimates: "Presupuestos", new: "Nuevo", finance: "Finanzas", clients: "Clientes", settings: "Ajustes" },

  auth: {
    signIn: "Iniciar sesión",
    signUp: "Crear cuenta",
    email: "Email",
    password: "Contraseña",
    fullName: "Tu nombre",
    noAccount: "¿Sin cuenta? Crea una",
    hasAccount: "¿Ya tienes cuenta? Entra",
    checkEmail: "Revisa tu email para confirmar la cuenta.",
    signOut: "Salir",
  },

  dashboard: {
    greeting: "Hola",
    newEstimate: "Nuevo presupuesto",
    pipeline: "Pipeline",
    recentEstimates: "Presupuestos recientes",
    noEstimates: "Aún no hay presupuestos. ¡Crea el primero!",
    thisMonth: "Este mes",
    approved: "Aprobados",
    pending: "Pendientes",
  },

  trades: {
    flooring: "Pisos",
    painting: "Pintura",
    drywall: "Drywall",
    tile: "Azulejo",
    roofing: "Techado",
    framing: "Framing (estructura)",
    trim: "Trim / acabado",
    siding: "Siding (revestimiento exterior)",
    concrete: "Concreto",
    remodeling: "Remodelación",
    landscaping: "Jardinería",
    cleaning: "Limpieza",
    finish_basement: "Acabado de sótano",
    handyman: "Mantenimiento",
  },

  form: {
    aiSoon: "Asistente IA muy pronto — fotos, voz y chat. Por ahora, ingresa los datos del trabajo abajo.",
    trade: "Tipo de trabajo",
    jobTitle: "Título del trabajo (opcional)",
    jobTitlePlaceholder: "ej: Piso vinílico en el cuarto",
    areas: "Áreas",
    areaName: "Nombre (opcional)",
    length: "Largo (ft)",
    width: "Ancho (ft)",
    addArea: "Agregar área",
    totalSqft: "O total en sqft (reemplaza las áreas)",
    wallHeight: "Altura de pared (ft)",
    tier: "Nivel de calidad",
    tierBasic: "Básico",
    tierStandard: "Estándar",
    tierPremium: "Premium",
    conditions: "Condiciones del trabajo",
    demo: "Quitar superficie existente (demolición)",
    prep: "Necesita preparación / reparación",
    disposal: "Retirar escombros",
    clientName: "Nombre del cliente (opcional)",
    calculate: "Calcular presupuesto",
    needArea: "Ingresa al menos una medida de área",
  },

  measure: {
    title: "Medidas",
    mode: { area: "Área", wall: "Área de pared", linear: "Pies lineales", footprint: "Proyección" },
    linearFeet: "Pies lineales (cerca, zócalo…)",
    needLinear: "Ingresa los pies lineales",
    footprintHint: "Ingresa la proyección del techo (planta); la pendiente se aplica automáticamente.",
    pitchLabel: "Pendiente del techo",
    pitch: { flat: "Plano", low: "Baja", medium: "Media", steep: "Empinada" },
    depthLabel: "Espesor de losa",
    doors: "Puertas",
    windows: "Ventanas",
    egress: "Ventanas de emergencia (1 por dormitorio — código)",
    bathroom: "Incluir rough-in de baño / wet-bar",
    basementNote: "Alcance por código: techo 2,13m, salida de emergencia, aislamiento R-15, GFCI/AFCI, barrera de humedad. Resuelva el agua antes de terminar.",
  },

  related: {
    title: "Trabajos relacionados a proponer",
    subtitle: "Una solicitud suele esconder un trabajo mayor. Plantéalos al cliente.",
    often: "Suele ir junto",
    check: "Vale revisar",
  },

  demand: {
    title: "Demanda por zona",
    subtitle: "De dónde vienen los trabajos y qué se vende — tus propios datos, creciendo con cada presupuesto.",
    emptyTitle: "Aún sin datos de ubicación",
    emptyBody: "Agrega la dirección en los presupuestos y este mapa se llena: qué ZIP y oficios traen más trabajo, y dónde ganas.",
    emptyCta: "Crear presupuesto",
    topArea: "Zona más caliente:",
    jobs: "trabajos",
    winRate: "conversión",
    byArea: "Top zonas por valor",
    byAreaHint: "Nº de trabajos · valor total cotizado por ZIP / ciudad.",
    byTrade: "Top oficios por valor",
    byTradeHint: "Lo que más se vende en todas las zonas.",
    winByArea: "Conversión por zona",
    noDecided: "Aún sin ganados/perdidos — la conversión aparece cuando los presupuestos se aprueban o pierden.",
    coverage: "{located} de {total} presupuestos tienen zona. Agrega direcciones para afinar esto.",
  },

  permits: {
    title: "Pulso del mercado — permisos de construcción",
    subtitle: "Demanda de construcción residencial en EE. UU.",
    national: "Estados Unidos · nacional",
    regionPrefix: "Tu región:",
    regions: { US: "Estados Unidos", NO: "Noreste", MW: "Medio Oeste", SO: "Sur", WE: "Oeste" } as Record<string, string>,
    permitsTotal: "Permisos · todas las unidades",
    permitsSingle: "Permisos · vivienda unifamiliar",
    startsTotal: "Inicios de vivienda",
    unit: "K",
    yoy: "vs año ant.",
    unavailable: "Datos del Census no disponibles ahora.",
    source: "Fuente: U.S. Census Bureau — Nueva Construcción Residencial, ajustado estacionalmente",
    updated: "Actualizado",
    needsKey:
      "Agrega una clave gratuita del Census (CENSUS_API_KEY) para ver las tendencias nacionales de permisos.",
    getKey: "Obtener clave gratuita del Census →",
  },

  newflow: {
    chooseTitle: "¿Qué quieres comenzar?",
    startResidential: "Presupuesto residencial",
    startResidentialDesc: "Un trabajo en casa — remodelación, reparación, ampliación.",
    startCommercial: "Presupuesto comercial",
    startCommercialDesc: "Un trabajo comercial — código, permisos, retención.",
    startProject: "Nuevo proyecto",
    startProjectDesc: "Un contenedor para varios trabajos de un cliente u obra.",
    types: { residential: "Presupuesto residencial", commercial: "Presupuesto comercial" } as Record<string, string>,
    typeSubtitle: {
      residential: "Trabajo en casa. El asesor adapta preguntas y reglas a lo residencial.",
      commercial: "Trabajo comercial. Aplican código, permisos, seguros y pagos más estrictos.",
    } as Record<string, string>,
    materialsQ: "¿El material está incluido en el presupuesto?",
    materialsHint: "Apagado = el cliente pone el material. Cambia lo que pregunta el asesor.",
    rulesTitle: "Reglas y requisitos para este tipo",
    code: "Base de código",
    permit: "Permiso e inspecciones",
    payment: "Cronograma de pago",
    retainage: "Retención",
    prereqs: "Prerrequisitos",
    risks: "Riesgos a cotizar",
    suggests: "Sugerido: {margin}% margen · {contingency}% contingencia",
    advisorTitle: "Asesor — preguntas para la obra",
    advisorHint: "Léelas a quien maneja la obra para capturar todo por adelantado.",
    answerPlaceholder: "Respuesta / notas…",
    categories: {
      scope: "alcance",
      site: "sitio",
      materials: "material",
      labor: "mano de obra",
      schedule: "cronograma",
      compliance: "cumplimiento",
      risk: "riesgo",
    } as Record<string, string>,
    newProjectTitle: "Nuevo proyecto",
    projectDetails: "Detalles del proyecto",
    projectName: "Nombre del proyecto",
    projectNamePlaceholder: "ej. Renovación calle Maple",
    projectType: "Tipo",
    projectTypes: { residential: "Residencial", commercial: "Comercial", mixed: "Mixto" } as Record<string, string>,
    client: "Cliente (opcional)",
    clientPlaceholder: "Nombre del cliente o empresa",
    address: "Dirección (opcional)",
    addressPlaceholder: "Calle, ciudad, estado",
    description: "Descripción (opcional)",
    descriptionPlaceholder: "¿De qué trata este proyecto?",
    createProject: "Crear proyecto",
    editProject: "Editar proyecto",
    edit: "Editar",
    saveChanges: "Guardar cambios",
    addressLine: "Dirección",
    city: "Ciudad",
    state: "Estado",
    zip: "ZIP",
    addressAutofillHint: "Empieza a escribir la dirección — elige una sugerencia y ciudad, estado y ZIP se completan solos.",
    projectsTitle: "Proyectos",
    projectsSubtitle: "Agrupa varios trabajos bajo un proyecto y obra.",
    newProject: "Nuevo",
    noProjects: "Aún no hay proyectos",
    noProjectsHint: "Crea un proyecto para agrupar trabajos de un cliente u obra.",
    jobs: "trabajos",
    contractTotal: "Total del contrato",
    jobsInProject: "Trabajos en este proyecto",
    addJob: "Agregar trabajo",
    noJobs: "Aún sin trabajos — agrega el primero.",
  },

  wizard: {
    tabGuided: "Guiado",
    tabQuick: "Rápido",
    kindTitle: "¿Qué tipo de proyecto?",
    residential: "Residencial",
    residentialDesc: "Casas, townhouses, apartamentos",
    commercial: "Comercial",
    commercialDesc: "Oficinas, tiendas, restaurantes",
    propertyTitle: "Tipo de propiedad",
    property: {
      single_house: "Casa (single house)",
      double_house: "Casa doble / dúplex",
      townhouse: "Townhouse",
      apartment: "Apartamento",
      mansion: "Mansión",
      office: "Oficina",
      retail: "Tienda",
      restaurant: "Restaurante",
      warehouse: "Bodega",
    },
    roomsTitle: "Mapea la propiedad",
    roomsHint: "Ajusta las cantidades — los tamaños vienen después.",
    rooms: {
      bedroom: "Cuarto",
      suite: "Suite",
      bathroom: "Baño",
      half_bath: "Medio baño",
      kitchen: "Cocina",
      living: "Sala",
      dining: "Comedor",
      hallway: "Pasillo",
      closet: "Clóset",
      laundry: "Lavandería",
      garage: "Garaje",
      office_room: "Oficina",
      open_area: "Área abierta",
    },
    areasTitle: "¿Qué áreas se trabajarán?",
    areasHint: "Toca para incluir o excluir. Ajusta los tamaños si hace falta.",
    selectAll: "Todas",
    clearAll: "Ninguna",
    worksTitle: "¿Qué trabajos se harán?",
    worksHint: "Selecciona todos los tipos que apliquen.",
    metalTrim: "Trim metálico (aluminio)",
    handymanTitle: "Tareas de mantenimiento",
    handymanHint: "Elige cada tarea, la cantidad y dónde en la propiedad.",
    wholeProperty: "Toda la propiedad",
    locationLabel: "Ubicación del trabajo",
    locationPlaceholder: "Dirección, código ZIP o ciudad",
    locationHint: "Los precios se ajustan por región — ej: Philadelphia ≠ King of Prussia.",
    costIndex: "índice de costo",
    detailsTitle: "Detalles del trabajo",
    review: "Revisar",
    reviewTitle: "Revisar y calcular",
    summaryAreas: "áreas",
    summaryWorks: "tipos de trabajo",
    totalArea: "Área total",
    next: "Siguiente",
    back: "Atrás",
  },

  analysis: {
    title: "Análisis del proyecto",
    riskTitle: "Nivel de riesgo",
    risk: { low: "Bajo", medium: "Medio", high: "Alto" },
    contingency: "Contingencia sugerida",
    factors: {
      demo: "Incluye demolición / remoción",
      multiTrade: "Varios tipos de trabajo por coordinar",
      tightMargin: "Margen debajo de tu meta saludable",
      largeArea: "Área grande de proyecto",
      estimatedPrices: "Muchos precios estimados — confirma tu lista de precios",
    },
    wasteTitle: "Análisis de desperdicio",
    wasteCost: "Costo de material en desperdicio",
    scheduleTitle: "Cronograma",
    crew: "cuadrilla",
    laborPerDay: "Costo de mano de obra por día",
    paymentsTitle: "Plan de pagos",
    deposit: "Depósito para agendar",
    progress: "Pago intermedio",
    final: "Final al terminar",
    competitorsSoon: "Guarda el presupuesto para activar la inteligencia de mercado con IA",
  },

  handyman: {
    door_repair: "Ajuste / reparación de puerta",
    door_install: "Instalación de puerta interior",
    faucet: "Cambio de grifo",
    toilet: "Cambio de inodoro",
    caulking: "Sellado de tina / regadera",
    drywall_patch: "Parche de drywall",
    light_fixture: "Cambio de lámpara",
    ceiling_fan: "Instalación de ventilador de techo",
    tv_mount: "Soporte de TV en pared",
    shelving: "Repisas / clóset",
    smoke_detector: "Detector de humo",
    weatherstripping: "Sellado de puertas/ventanas",
    baseboard_repair: "Reparación de zócalo",
    pressure_wash: "Lavado a presión",
    gutter_clean: "Limpieza de canaletas",
  },

  tasks: {
    title: "Tareas del trabajo",
    placeholder: "Nueva tarea… ej: comprar materiales",
    block: "Bloquear",
    unblock: "Desbloquear",
    status: {
      todo: "Por hacer",
      in_progress: "En curso",
      done: "Terminada",
      blocked: "Bloqueada",
    },
  },

  photos: {
    title: "Fotos antes y después",
    before: "Antes",
    after: "Después",
    add: "Añadir",
    empty: "Sin fotos aún",
  },

  alerts: {
    title: "Alarmas e incidentes",
    allClear: "Todo bien — sin incidentes.",
    onTrack: "En tiempo",
    attention: "Atención",
    incident: "Incidente",
    cashEvolution: "Evolución de caja (60 días)",
    timeline: "Línea de tiempo de proyectos",
    noProjects: "Sin proyectos activos.",
    start: "Inicio",
    forecastEnd: "Fin previsto",
    tasksDone: "tareas",
    kinds: {
      taskOverdue: "Tarea atrasada",
      taskDueSoon: "Tarea vence pronto",
      taskBlocked: "Tarea bloqueada",
      projectLate: "Proyecto pasado del plazo",
      projectEndingSoon: "Proyecto por terminar",
      overBudget: "Gasto sobre el presupuesto",
      lowMargin: "Margen bajo",
    },
  },

  crm: {
    kanban: "Kanban",
    list: "Lista",
    pipeline: "Pipeline",
    openDeals: "Tratos abiertos",
    winRate: "Tasa de cierre",
    funnel: "Pipeline por etapa",
    emptyStage: "Vacío",
  },


  ai: {
    tab: "IA",
    description: "Descripción del trabajo",
    descriptionPlaceholder: "ej: casa de 2 pisos, siding de vinilo, moho en el lado norte…",
    measurements: "Medidas conocidas (opcional)",
    measurementsPlaceholder: "ej: driveway 20×40 ft, deck 300 sqft",
    improve: "Mejorar con IA",
    area: "Área",
    areaName: "Nombre del área",
    addArea: "Agregar área",
    orDirect: "o",
    photoLink: "Foto",
    noPhoto: "Sin foto",
    photo: "Foto",
    photos: "Fotos del trabajo",
    generate: "Generar borrador con IA",
    generating: "Leyendo fotos y armando borrador…",
    needInput: "Agrega una descripción o al menos una foto",
    disclaimer: "Borrador generado por IA. El precio final debe ser revisado y aprobado por el contratista.",
    scope: "Alcance detectado",
    assumptions: "Supuestos",
    warnings: "Verificar en sitio",
    lineItems: "Partidas (costos)",
    equipment: "Equipos",
    totalCost: "Costo total",
    prices: { minimum: "Mínimo", recommended: "Recomendado", premium: "Premium" },
    accept: "Guardar como presupuesto (revisar después)",
  },

  profit: {
    title: "Protección de ganancia",
    ok: "Ganancia proyectada {margin}% — sobre tu mínimo.",
    low: "Este presupuesto tiene solo {margin}% de ganancia proyectada. Mínimo recomendado: {min}%. Precio final sugerido: {price}.",
  },

  billing: {
    title: "Cobros",
    invoices: "Facturas",
    newInvoice: "Nueva factura",
    invoicePlaceholder: "ej: Depósito",
    paymentLink: "Link de pago",
    markPaid: "Marcar pagada",
    void: "Anular",
    invStatus: { unpaid: "Pendiente", paid: "Pagada", void: "Anulada" },
    needsStripe: "Agrega STRIPE_SECRET_KEY para activar links de pago (panel Stripe → API keys).",
    changeOrders: "Órdenes de cambio",
    newCo: "Nueva orden de cambio",
    coPlaceholder: "ej: Agregar pared del garaje",
    approve: "Aprobar",
    decline: "Rechazar",
    coStatus: { pending: "Pendiente", approved: "Aprobada", declined: "Rechazada" },
    totalWithCo: "Contrato + órdenes aprobadas",
  },

  share: {
    savePdf: "Guardar PDF",
    whatsappTitle: "Enviar por WhatsApp",
    phone: "Número de teléfono",
    phoneHint: "Cualquier número — en EE.UU. puedes omitir el +1.",
    send: "Abrir WhatsApp",
    printFooter: "Presupuesto válido por 30 días. Precios sujetos a confirmación en sitio.",
  },

  finance: {
    title: "Finanzas",
    add: "Registrar",
    save: "Guardar",
    added: "¡Guardado!",
    empty: "Aún no hay movimientos. Controla gastos y cobros por trabajo.",
    income: "Entradas",
    expenses: "Salidas",
    balance: "Saldo",
    incomeOne: "Entrada",
    expense: "Salida",
    category: "Categoría",
    amount: "Monto",
    date: "Fecha",
    description: "Descripción (opcional)",
    descriptionPlaceholder: "ej: gasolina de la troca",
    linkedJob: "Trabajo (opcional)",
    noJob: "Sin trabajo — general",
    invalidAmount: "Ingresa un monto válido",
    categories: {
      materials: "Materiales",
      labor: "Pago a empleados",
      fuel: "Gasolina / combustible",
      equipment: "Equipo / herramientas",
      disposal: "Escombros / contenedor",
      permits: "Permisos / tarifas",
      meals: "Comidas",
      other: "Otros",
      deposit: "Depósito",
      progress: "Pago intermedio",
      final: "Pago final",
      other_income: "Otra entrada",
    },
    jobCostTitle: "Costo del trabajo",
    seeAll: "Ver todo",
    contract: "Precio del contrato",
    received: "Recibido",
    outstanding: "Por cobrar",
    estCost: "Costo estimado",
    actualSpent: "Gasto real",
    projectedProfit: "Ganancia proyectada",
    realProfit: "Ganancia real hasta ahora",
  },

  home: {
    revenueMonth: "Ingresos (mes)",
    profitMonth: "Ganancia (mes)",
    cash: "Caja",
    pipeline: "Pipeline",
    winRate: "Cierre",
    outstanding: "Por cobrar",
    activeJobs: "Trabajos activos",
    revenueByMonth: "Ingresos por mes",
    estimatePipeline: "Pipeline de presupuestos",
    bucketDraft: "Borradores",
    bucketSent: "Enviados",
    bucketWon: "Ganados",
  },

  market: {
    title: "Inteligencia de mercado",
    intro: "La IA compara tu precio con trabajos similares en tu región, el estado y las top companies.",
    analyze: "Analizar mercado",
    analyzing: "Analizando mercado…",
    refresh: "Actualizar",
    region: "Tu región",
    state: "Promedio estatal",
    topCompanies: "Top companies",
    competitors: "Perfiles de competidores",
    avg: "Promedio",
    ourPrice: "Tu precio",
    priceToBeat: "Precio para ganar",
    needsKey: "Falta la clave de IA. Agrega AI_GATEWAY_API_KEY en .env.local (o despliega en Vercel, donde el AI Gateway funciona automático).",
    disclaimer: "Estimaciones de mercado por IA para planificación — no son cotizaciones reales. Confirma localmente antes de ajustar tu propuesta.",
  },

  chat: {
    title: "Nuevo presupuesto",
    placeholder: "Describe el trabajo… ej: “cuarto 12 por 14, piso vinílico, quitar alfombra”",
    send: "Enviar",
    listening: "Escuchando…",
    addPhoto: "Agregar foto",
    voice: "Voz",
    thinking: "Pensando…",
    calculating: "Calculando presupuesto…",
    checkingPrices: "Consultando tu lista de precios…",
    saveEstimate: "Guardar presupuesto",
    saving: "Guardando…",
    intro: "Cuéntame del trabajo — tipo, tamaño, fotos o voz. Yo armo el presupuesto.",
    estimateReady: "Presupuesto listo",
  },

  estimate: {
    title: "Presupuesto",
    material: "Material",
    labor: "Mano de obra",
    demoDisposal: "Demolición y desecho",
    other: "Otros",
    subtotal: "Subtotal de costo",
    overhead: "Gastos generales",
    profit: "Ganancia",
    tax: "Impuesto",
    total: "Precio total",
    marginScore: "Margen",
    healthy: "Saludable",
    medium: "Ajustada",
    low: "Baja — ¡revisa!",
    crew: "Equipo",
    people: "personas",
    days: "días",
    area: "Área",
    addItem: "Agregar ítem",
    deleteItem: "Eliminar",
    qty: "Cant",
    unit: "Unidad",
    unitCost: "Costo unit.",
    itemTotal: "Total",
    description: "Descripción",
    estimatedPrice: "precio estimado",
    generateProposal: "Generar propuesta",
    status: { draft: "Borrador", ready: "Listo", sent: "Enviado", approved: "Aprobado", lost: "Perdido", ai_generated: "Borrador IA", change_requested: "Cambio pedido", job: "En obra" },
    client: "Cliente",
    noClient: "Sin cliente",
    saved: "Guardado",
  },

  proposal: {
    title: "Propuesta",
    scope: "Alcance del trabajo",
    exclusions: "Exclusiones",
    terms: "Términos y condiciones",
    validUntil: "Válida hasta",
    generate: "Generar texto",
    generating: "Escribiendo propuesta…",
    send: "Enviar al cliente",
    copyLink: "Copiar enlace",
    linkCopied: "¡Enlace copiado!",
    downloadPdf: "Descargar PDF",
    preparedFor: "Preparada para",
    preparedBy: "Preparada por",
    acceptTitle: "Aceptar esta propuesta",
    acceptName: "Escribe tu nombre completo para firmar",
    acceptButton: "Aceptar y firmar",
    accepted: "Propuesta aceptada",
    acceptedOn: "Aceptada el",
    expired: "Esta propuesta expiró.",
    notFound: "Propuesta no encontrada.",
    investment: "Inversión",
    estimatedDuration: "Duración estimada",
    viewed: "Vista",
  },

  clients: {
    title: "Clientes",
    add: "Agregar cliente",
    name: "Nombre",
    phone: "Teléfono",
    email: "Email",
    address: "Dirección",
    notes: "Notas",
    save: "Guardar",
    empty: "Aún no hay clientes.",
    daysAgo: "d atrás",
    status: {
      lead: "Lead",
      estimate_sent: "Presupuesto enviado",
      follow_up: "Seguimiento",
      approved: "Aprobado",
      lost: "Perdido",
    },
  },

  prices: {
    title: "Lista de precios",
    add: "Agregar ítem",
    item: "Ítem",
    materialCost: "Material",
    laborCost: "Mano de obra",
    perUnit: "por",
    save: "Guardar",
    customPrice: "Tu precio",
    defaultPrice: "Predeterminado",
    hint: "Tus precios reemplazan los predeterminados en cada presupuesto.",
  },

  settings: {
    title: "Ajustes",
    company: "Nombre de la empresa",
    yourName: "Tu nombre",
    phone: "Teléfono",
    language: "Idioma",
    overhead: "Gastos generales %",
    profit: "Ganancia %",
    tax: "Impuesto %",
    minMargin: "Margen mínimo saludable %",
    save: "Guardar",
    saved: "¡Guardado!",
    numbers: "Tus números",
    numbersHint: "Usados como predeterminados en cada nuevo presupuesto.",
  },

  common: {
    loading: "Cargando…",
    error: "Algo salió mal",
    cancel: "Cancelar",
    delete: "Eliminar",
    edit: "Editar",
    back: "Volver",
  },
};
