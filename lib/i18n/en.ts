export const en = {
  appName: "ContractorOS AI",
  tagline: "Photos, measurements or voice → professional estimate in minutes",

  nav: { home: "Home", estimates: "Estimates", new: "New", finance: "Finance", clients: "Clients", settings: "Settings" },

  auth: {
    signIn: "Sign in",
    signUp: "Create account",
    email: "Email",
    password: "Password",
    fullName: "Your name",
    noAccount: "No account? Create one",
    hasAccount: "Already have an account? Sign in",
    checkEmail: "Check your email to confirm your account.",
    signOut: "Sign out",
  },

  dashboard: {
    greeting: "Hey",
    newEstimate: "New estimate",
    pipeline: "Pipeline",
    recentEstimates: "Recent estimates",
    noEstimates: "No estimates yet. Create your first one!",
    thisMonth: "This month",
    approved: "Approved",
    pending: "Pending",
  },

  trades: {
    flooring: "Flooring",
    painting: "Painting",
    drywall: "Drywall",
    tile: "Tile",
    roofing: "Roofing",
    framing: "Framing",
    trim: "Trim / finish carpentry",
    siding: "Siding",
    concrete: "Concrete",
    remodeling: "Remodeling",
    landscaping: "Landscaping",
    cleaning: "Cleaning",
    finish_basement: "Finish basement",
    handyman: "Handyman",
  } as Record<string, string>,

  form: {
    aiSoon: "AI assistant coming soon — photos, voice and chat. For now, enter the job details below.",
    trade: "Type of work",
    jobTitle: "Job title (optional)",
    jobTitlePlaceholder: "e.g. Bedroom LVP flooring",
    areas: "Areas",
    areaName: "Name (optional)",
    length: "Length (ft)",
    width: "Width (ft)",
    addArea: "Add area",
    totalSqft: "Or total sqft (overrides areas)",
    wallHeight: "Wall height (ft)",
    tier: "Quality tier",
    tierBasic: "Basic",
    tierStandard: "Standard",
    tierPremium: "Premium",
    conditions: "Job conditions",
    demo: "Remove existing surface (demo)",
    prep: "Prep / repair needed",
    disposal: "Haul away debris",
    clientName: "Client name (optional)",
    calculate: "Calculate estimate",
    needArea: "Enter at least one area dimension",
  },

  measure: {
    title: "Measurements",
    mode: { area: "Area", wall: "Wall area", linear: "Linear ft", footprint: "Footprint" } as Record<string, string>,
    linearFeet: "Linear feet (fence, trim run…)",
    needLinear: "Enter the linear feet",
    footprintHint: "Enter the roof footprint (ground plan); pitch is applied automatically.",
    pitchLabel: "Roof pitch",
    pitch: { flat: "Flat", low: "Low", medium: "Medium", steep: "Steep" } as Record<string, string>,
    depthLabel: "Slab thickness",
    doors: "Doors",
    windows: "Windows",
    egress: "Egress windows (1 per bedroom — code)",
    bathroom: "Include a bathroom / wet-bar rough-in",
    basementNote: "Code-driven scope: 7ft ceiling, egress, R-15 insulation, GFCI/AFCI, moisture barrier. Fix any water before finishing.",
  },

  related: {
    title: "Related work to propose",
    subtitle: "One request usually hides a bigger job. Raise these with the client.",
    often: "Usually bundled",
    check: "Worth checking",
  },

  demand: {
    title: "Demand by area",
    subtitle: "Where your jobs come from and what sells — your own data, growing with every estimate.",
    emptyTitle: "No location data yet",
    emptyBody: "Add the property address on estimates and this map fills in: which ZIPs and trades bring the most work, and where you win.",
    emptyCta: "Create an estimate",
    topArea: "Hottest area:",
    jobs: "jobs",
    winRate: "win rate",
    byArea: "Top areas by value",
    byAreaHint: "Job count · total quoted value per ZIP / city.",
    byTrade: "Top trades by value",
    byTradeHint: "What sells most across all areas.",
    winByArea: "Win rate by area",
    noDecided: "No won/lost decisions yet — win rate appears once estimates are approved or lost.",
    coverage: "{located} of {total} estimates carry an area. Add addresses to sharpen this.",
  },

  permits: {
    title: "Market pulse — building permits",
    subtitle: "U.S. residential construction demand.",
    national: "United States · national",
    regionPrefix: "Your region:",
    regions: { US: "United States", NO: "Northeast", MW: "Midwest", SO: "South", WE: "West" } as Record<string, string>,
    permitsTotal: "Permits · all units",
    permitsSingle: "Permits · single-family",
    startsTotal: "Housing starts",
    unit: "K",
    yoy: "vs last yr",
    unavailable: "Census data unavailable right now.",
    source: "Source: U.S. Census Bureau — New Residential Construction, seasonally adjusted",
    updated: "Updated",
    needsKey:
      "Add a free Census API key (CENSUS_API_KEY) to show national building-permit trends.",
    getKey: "Get a free Census key →",
  },

  newflow: {
    chooseTitle: "What do you want to start?",
    startResidential: "Residential estimate",
    startResidentialDesc: "A home job — remodel, repair, addition.",
    startCommercial: "Commercial estimate",
    startCommercialDesc: "A business/commercial job — code, permits, retainage.",
    startProject: "New project",
    startProjectDesc: "A container for several jobs of one client or site.",
    types: { residential: "Residential estimate", commercial: "Commercial estimate" } as Record<string, string>,
    typeSubtitle: {
      residential: "Home job. The advisor tailors questions and rules to residential work.",
      commercial: "Business job. Stricter code, permits, insurance, and payment terms apply.",
    } as Record<string, string>,
    materialsQ: "Is material included in the estimate?",
    materialsHint: "Off = client supplies material. Changes what the advisor asks.",
    rulesTitle: "Rules & requirements for this type",
    code: "Code basis",
    permit: "Permit & inspections",
    payment: "Payment schedule",
    retainage: "Retainage",
    prereqs: "Prerequisites",
    risks: "Risks to price for",
    suggests: "Suggested: {margin}% margin · {contingency}% contingency",
    advisorTitle: "Advisor — questions for the site",
    advisorHint: "Read these to whoever runs the site to capture everything up-front.",
    answerPlaceholder: "Answer / notes…",
    categories: {
      scope: "scope",
      site: "site",
      materials: "materials",
      labor: "labor",
      schedule: "schedule",
      compliance: "compliance",
      risk: "risk",
    } as Record<string, string>,
    newProjectTitle: "New project",
    projectDetails: "Project details",
    projectName: "Project name",
    projectNamePlaceholder: "e.g. Maple St. renovation",
    projectType: "Type",
    projectTypes: { residential: "Residential", commercial: "Commercial", mixed: "Mixed" } as Record<string, string>,
    client: "Client (optional)",
    clientPlaceholder: "Client or company name",
    address: "Address (optional)",
    addressPlaceholder: "Street, city, state",
    description: "Description (optional)",
    descriptionPlaceholder: "What is this project about?",
    createProject: "Create project",
    editProject: "Edit project",
    edit: "Edit",
    saveChanges: "Save changes",
    addressLine: "Street address",
    city: "City",
    state: "State",
    zip: "ZIP",
    addressAutofillHint: "Start typing the address — pick a suggestion and city, state and ZIP fill in automatically.",
    projectsTitle: "Projects",
    projectsSubtitle: "Group several jobs under one project and site.",
    newProject: "New",
    noProjects: "No projects yet",
    noProjectsHint: "Create a project to group related jobs of a client or site.",
    jobs: "jobs",
    contractTotal: "Contract total",
    jobsInProject: "Jobs in this project",
    addJob: "Add job",
    noJobs: "No jobs yet — add the first one.",
  },

  wizard: {
    tabGuided: "Guided",
    tabQuick: "Quick",
    kindTitle: "What kind of project?",
    residential: "Residential",
    residentialDesc: "Houses, townhouses, apartments",
    commercial: "Commercial",
    commercialDesc: "Offices, retail, restaurants",
    propertyTitle: "Property type",
    property: {
      single_house: "Single house",
      double_house: "Double house / duplex",
      townhouse: "Townhouse",
      apartment: "Apartment",
      mansion: "Mansion / estate",
      office: "Office",
      retail: "Retail store",
      restaurant: "Restaurant",
      warehouse: "Warehouse",
    } as Record<string, string>,
    roomsTitle: "Map the property",
    roomsHint: "Adjust the counts — sizes come next.",
    rooms: {
      bedroom: "Bedroom",
      suite: "Suite",
      bathroom: "Bathroom",
      half_bath: "Half bath",
      kitchen: "Kitchen",
      living: "Living room",
      dining: "Dining room",
      hallway: "Hallway",
      closet: "Closet",
      laundry: "Laundry",
      garage: "Garage",
      office_room: "Office / den",
      open_area: "Open area",
    } as Record<string, string>,
    areasTitle: "Which areas will be worked?",
    areasHint: "Tap to include or exclude. Adjust sizes as needed.",
    selectAll: "All",
    clearAll: "None",
    worksTitle: "What work will be done?",
    worksHint: "Select every work type that applies.",
    metalTrim: "Metal trim (aluminum)",
    handymanTitle: "Handyman tasks",
    handymanHint: "Pick each task, how many, and where in the property.",
    wholeProperty: "Whole property",
    locationLabel: "Job location",
    locationPlaceholder: "Address, ZIP code or city",
    locationHint: "Prices adjust by region — e.g. Philadelphia ≠ King of Prussia.",
    costIndex: "cost index",
    detailsTitle: "Job details",
    review: "Review",
    reviewTitle: "Review & calculate",
    summaryAreas: "areas",
    summaryWorks: "work types",
    totalArea: "Total area",
    next: "Next",
    back: "Back",
  },

  analysis: {
    title: "Project analysis",
    riskTitle: "Risk level",
    risk: { low: "Low", medium: "Medium", high: "High" } as Record<string, string>,
    contingency: "Suggested contingency",
    factors: {
      demo: "Demolition / removal involved",
      multiTrade: "Multiple work types to coordinate",
      tightMargin: "Margin below your healthy target",
      largeArea: "Large project area",
      estimatedPrices: "Many prices are estimates — confirm your price book",
    } as Record<string, string>,
    wasteTitle: "Waste analysis",
    wasteCost: "Material cost in waste",
    scheduleTitle: "Work schedule",
    crew: "crew",
    laborPerDay: "Labor cost per day",
    paymentsTitle: "Payment plan",
    deposit: "Deposit to schedule",
    progress: "Progress payment",
    final: "Final on completion",
    competitorsSoon: "Save the estimate to unlock AI market intelligence",
  },

  handyman: {
    door_repair: "Door adjust / repair",
    door_install: "Interior door install",
    faucet: "Faucet replacement",
    toilet: "Toilet replacement",
    caulking: "Tub / shower caulking",
    drywall_patch: "Drywall patch",
    light_fixture: "Light fixture swap",
    ceiling_fan: "Ceiling fan install",
    tv_mount: "TV wall mount",
    shelving: "Shelving / closet install",
    smoke_detector: "Smoke detector install",
    weatherstripping: "Weatherstripping",
    baseboard_repair: "Baseboard repair",
    pressure_wash: "Pressure washing",
    gutter_clean: "Gutter cleaning",
  } as Record<string, string>,

  tasks: {
    title: "Job tasks",
    placeholder: "New task… e.g. order materials",
    block: "Block",
    unblock: "Unblock",
    status: {
      todo: "To do",
      in_progress: "In progress",
      done: "Done",
      blocked: "Blocked",
    } as Record<string, string>,
  },

  photos: {
    title: "Before & after photos",
    before: "Before",
    after: "After",
    add: "Add",
    empty: "No photos yet",
  },

  alerts: {
    title: "Alarms & incidents",
    allClear: "All clear — no incidents.",
    onTrack: "On track",
    attention: "Attention",
    incident: "Incident",
    cashEvolution: "Cash evolution (60 days)",
    timeline: "Project timeline",
    noProjects: "No active projects.",
    start: "Start",
    forecastEnd: "Forecast end",
    tasksDone: "tasks",
    kinds: {
      taskOverdue: "Task overdue",
      taskDueSoon: "Task due soon",
      taskBlocked: "Task blocked",
      projectLate: "Project past forecast end",
      projectEndingSoon: "Project ending soon",
      overBudget: "Spending over estimate",
      lowMargin: "Low margin",
    } as Record<string, string>,
  },

  crm: {
    kanban: "Kanban",
    list: "List",
    pipeline: "Pipeline",
    openDeals: "Open deals",
    winRate: "Win rate",
    funnel: "Pipeline by stage",
    emptyStage: "Empty",
  },


  ai: {
    tab: "AI",
    description: "Job description",
    descriptionPlaceholder: "e.g. two-story house, vinyl siding, mildew on the north side…",
    measurements: "Known measurements (optional)",
    measurementsPlaceholder: "e.g. driveway 20×40 ft, deck 300 sqft",
    improve: "Improve with AI",
    area: "Area",
    areaName: "Area name",
    addArea: "Add area",
    orDirect: "or",
    photoLink: "Photo",
    noPhoto: "No photo",
    photo: "Photo",
    photos: "Job photos",
    generate: "Generate AI draft",
    generating: "Reading photos & building draft…",
    needInput: "Add a description or at least one photo",
    disclaimer: "AI-generated draft. Final price must be reviewed and approved by the contractor.",
    scope: "Detected scope",
    assumptions: "Assumptions",
    warnings: "Verify on site",
    lineItems: "Line items (costs)",
    equipment: "Equipment",
    totalCost: "Total cost",
    prices: { minimum: "Minimum", recommended: "Recommended", premium: "Premium" } as Record<string, string>,
    accept: "Save as estimate (review next)",
  },

  profit: {
    title: "Profit protection",
    ok: "Projected profit {margin}% — above your minimum.",
    low: "This estimate has only {margin}% projected profit. Recommended minimum is {min}%. Suggested final price: {price}.",
  },

  billing: {
    title: "Billing",
    invoices: "Invoices",
    newInvoice: "New invoice",
    invoicePlaceholder: "e.g. Deposit",
    paymentLink: "Payment link",
    markPaid: "Mark paid",
    void: "Void",
    invStatus: { unpaid: "Unpaid", paid: "Paid", void: "Void" } as Record<string, string>,
    needsStripe: "Add STRIPE_SECRET_KEY to enable payment links (Stripe dashboard → API keys).",
    changeOrders: "Change orders",
    newCo: "New change order",
    coPlaceholder: "e.g. Add garage wall",
    approve: "Approve",
    decline: "Decline",
    coStatus: { pending: "Pending", approved: "Approved", declined: "Declined" } as Record<string, string>,
    totalWithCo: "Contract + approved change orders",
  },

  share: {
    savePdf: "Save PDF",
    whatsappTitle: "Send via WhatsApp",
    phone: "Phone number",
    phoneHint: "Any number — US numbers can skip the +1.",
    send: "Open WhatsApp",
    printFooter: "Estimate valid for 30 days. Prices subject to site confirmation.",
  },

  finance: {
    title: "Finance",
    add: "Add",
    save: "Save",
    added: "Saved!",
    empty: "No transactions yet. Track expenses and payments per job.",
    income: "In",
    expenses: "Out",
    balance: "Balance",
    incomeOne: "Income",
    expense: "Expense",
    category: "Category",
    amount: "Amount",
    date: "Date",
    description: "Description (optional)",
    descriptionPlaceholder: "e.g. gas for the truck",
    linkedJob: "Job (optional)",
    noJob: "No job — general",
    invalidAmount: "Enter a valid amount",
    categories: {
      materials: "Materials",
      labor: "Employee pay",
      fuel: "Fuel / gas",
      equipment: "Equipment / tools",
      disposal: "Disposal / dump",
      permits: "Permits / fees",
      meals: "Meals",
      other: "Other",
      deposit: "Deposit",
      progress: "Progress payment",
      final: "Final payment",
      other_income: "Other income",
    } as Record<string, string>,
    jobCostTitle: "Job costing",
    seeAll: "See all",
    contract: "Contract price",
    received: "Received",
    outstanding: "Outstanding",
    estCost: "Estimated cost",
    actualSpent: "Actual spent",
    projectedProfit: "Projected profit",
    realProfit: "Real profit so far",
  },

  home: {
    revenueMonth: "Revenue (mo)",
    profitMonth: "Profit (mo)",
    cash: "Cash",
    pipeline: "Pipeline",
    winRate: "Win rate",
    outstanding: "Owed to you",
    activeJobs: "Active jobs",
    revenueByMonth: "Revenue by month",
    estimatePipeline: "Estimate pipeline",
    bucketDraft: "Drafts",
    bucketSent: "Sent",
    bucketWon: "Won",
  },

  market: {
    title: "Market intelligence",
    intro: "AI compares your price with similar jobs in your region, state and top companies.",
    analyze: "Analyze market",
    analyzing: "Analyzing market…",
    refresh: "Refresh",
    region: "Your region",
    state: "State average",
    topCompanies: "Top companies",
    competitors: "Competitor profiles",
    avg: "Average",
    ourPrice: "Your price",
    priceToBeat: "Price to beat",
    needsKey: "AI key missing. Add AI_GATEWAY_API_KEY to .env.local (or deploy on Vercel, where the AI Gateway works automatically).",
    disclaimer: "AI market estimates for planning — not real quotes. Confirm locally before adjusting your bid.",
  },

  chat: {
    title: "New estimate",
    placeholder: "Describe the job… e.g. “bedroom 12 by 14, vinyl plank, remove carpet”",
    send: "Send",
    listening: "Listening…",
    addPhoto: "Add photo",
    voice: "Voice",
    thinking: "Thinking…",
    calculating: "Calculating estimate…",
    checkingPrices: "Checking your price book…",
    saveEstimate: "Save estimate",
    saving: "Saving…",
    intro: "Tell me about the job — type, size, photos or voice. I'll build the estimate.",
    estimateReady: "Estimate ready",
  },

  estimate: {
    title: "Estimate",
    material: "Material",
    labor: "Labor",
    demoDisposal: "Demo & disposal",
    other: "Other",
    subtotal: "Cost subtotal",
    overhead: "Overhead",
    profit: "Profit",
    tax: "Tax",
    total: "Total price",
    marginScore: "Margin",
    healthy: "Healthy",
    medium: "Tight",
    low: "Low — review!",
    crew: "Crew",
    people: "people",
    days: "days",
    area: "Area",
    addItem: "Add item",
    deleteItem: "Delete",
    qty: "Qty",
    unit: "Unit",
    unitCost: "Unit cost",
    itemTotal: "Total",
    description: "Description",
    estimatedPrice: "estimated price",
    generateProposal: "Generate proposal",
    status: { draft: "Draft", ready: "Ready", sent: "Sent", approved: "Approved", lost: "Lost", ai_generated: "AI draft", change_requested: "Change requested", job: "Job" } as Record<string, string>,
    client: "Client",
    noClient: "No client",
    saved: "Saved",
  },

  proposal: {
    title: "Proposal",
    scope: "Scope of work",
    exclusions: "Exclusions",
    terms: "Terms & conditions",
    validUntil: "Valid until",
    generate: "Generate text",
    generating: "Writing proposal…",
    send: "Send to client",
    copyLink: "Copy link",
    linkCopied: "Link copied!",
    downloadPdf: "Download PDF",
    preparedFor: "Prepared for",
    preparedBy: "Prepared by",
    acceptTitle: "Accept this proposal",
    acceptName: "Type your full name to sign",
    acceptButton: "Accept & sign",
    accepted: "Proposal accepted",
    acceptedOn: "Accepted on",
    expired: "This proposal has expired.",
    notFound: "Proposal not found.",
    investment: "Investment",
    estimatedDuration: "Estimated duration",
    viewed: "Viewed",
  },

  clients: {
    title: "Clients",
    add: "Add client",
    name: "Name",
    phone: "Phone",
    email: "Email",
    address: "Address",
    notes: "Notes",
    save: "Save",
    empty: "No clients yet.",
    daysAgo: "d ago",
    status: {
      lead: "Lead",
      estimate_sent: "Estimate sent",
      follow_up: "Follow up",
      approved: "Approved",
      lost: "Lost",
    },
  },

  prices: {
    title: "Price book",
    add: "Add item",
    item: "Item",
    materialCost: "Material",
    laborCost: "Labor",
    perUnit: "per",
    save: "Save",
    customPrice: "Your price",
    defaultPrice: "Default",
    hint: "Your prices override defaults in every estimate.",
  },

  settings: {
    title: "Settings",
    company: "Company name",
    yourName: "Your name",
    phone: "Phone",
    language: "Language",
    overhead: "Overhead %",
    profit: "Profit %",
    tax: "Tax %",
    minMargin: "Minimum healthy margin %",
    save: "Save",
    saved: "Saved!",
    numbers: "Your numbers",
    numbersHint: "Used as defaults on every new estimate.",
  },

  common: {
    loading: "Loading…",
    error: "Something went wrong",
    cancel: "Cancel",
    delete: "Delete",
    edit: "Edit",
    back: "Back",
  },
};

export type Dict = typeof en;
