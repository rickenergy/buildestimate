...
export function SettingsForm({ profile, email }: { profile: Profile; email: string }) {
  ...
  const [bannerZoom, setBannerZoom] = useState<number>(profile.banner_zoom ?? 100);
            ...
            <div className="flex h-24 w-full items-center justify-center overflow-hidden rounded-xl bg-muted ring-1 ring-foreground/10">
              {bannerUrl ? (
                ...
                <img
                  ...
                  style={{ objectPosition: `center ${bannerPos}%`, transform: `scale(${bannerZoom / 100})` }}
                />
              ) : (
                <ImagePlus className="h-5 w-5 text-muted-foreground" />
              )}
            ...
            {bannerUrl && (
              <>
                <div className="flex items-center gap-2">
                  <span className="w-20 shrink-0 text-[10px] text-muted-foreground">{tr(BRAND.reposition)}</span>
                  <input
                    type="range"
                    min={0}
                    max={100}
                ...
                <div className="flex items-center gap-2">
                  <span className="w-20 shrink-0 text-[10px] text-muted-foreground">{tr(BRAND.zoom)}</span>
                  <input
                    type="range"
                    min={100}
                    max={200}
                    ...
                    onMouseUp={() => startTransition(() => updateProfile({ banner_zoom: bannerZoom }))}
                    onTouchEnd={() => startTransition(() => updateProfile({ banner_zoom: bannerZoom }))}
                    className="h-1.5 flex-1 accent-primary"
                  />
                </div>
              </>
